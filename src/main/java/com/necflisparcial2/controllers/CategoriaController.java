...
import com.necflisparcial2.entities.Categoria;
import com.necflisparcial2.repositories.CategoriaRepository;
...
public class CategoriaController {
	...
	public Categoria updateCategoria(@PathVariable(name = "id") Long id, 
		...
		Optional<Categoria> updCategoria = repository.findById(id);
		if(updCategoria.isPresent()) {
			Categoria actual = updCategoria.get(); 
			actual.setId(id);
			actual.setNombre(categoria.getNombre());
			actual.setDescripcion(categoria.getDescripcion());
			actual.setFechacreacion(categoria.getFechacreacion());
